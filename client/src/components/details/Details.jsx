...
export default function Details() {
  ...
  useEffect(() => {
    storyService.getOne(storyId).then(setStory);

    commentService.getAll(storyId).then((result) => {
      dispatch({
  ...
  const deleteButtonClickHandler = async () => {
    const hasConfirmed = confirm(
      `Are you sure you want to delete your ${story.title}`
    );
      ...
      <div className="info-section">
        ...
        {userId === story._ownerId && (
          <div className="detailsBtn">
            <Link
              to={pathToUrl(Path.StoryEdit, { storyId })}
              className="button"
            >
              Edit
            </Link>
